"""
Single-machine HDFS dataset implementation.
"""
...
class CorgiPileHDFSDataset(IterableDataset):
    """
    Block-level dataset for HDFS with dual-layer shuffle.
    
    Features:
    - HDFS file system support via pyarrow
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Get all valid HDFS files
        self.data_files = self._list_hdfs_files()
        self.file_id_mapping = {file_path: idx for idx, file_path in enumerate(self.data_files)}
        
        print(f"Found {len(self.data_files)} files in HDFS path {hdfs_root}")
    ...
    def _list_hdfs_files(self) -> list:
        ...
        try:
            all_files = []
            for entry in self.hdfs.get_file_info(pyarrow.fs.FileSelector(self.hdfs_root, recursive=False)):
                if entry.is_file:
    ...
    def _create_block_iterator(self, worker_files: list, worker_id: int) -> Iterable[Tuple[Any, Any, Tuple[int, int]]]:
        ...
        print(f"Worker {worker_id} processing {len(worker_files)} HDFS files")
        ...
        for file_path in worker_files:
            ...
            for item in self.load_data_fn(
                file_path, 
                self.hdfs, 
                **{**self.kwargs, 'worker_id': worker_id, 'file_id': file_id}
            ):
